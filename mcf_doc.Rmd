...
```{r}
pdf("mcf_Figure2.pdf", width=10, height=6)
...
## Plot MCF by document type and issuing authority
...
network_plot_1720 <- correlations_filtered_1720 %>% 
  network_plot() + 
  labs(caption = "Data Source: MCF Documents Dataset Ver.1") +
  theme(plot.title = element_text(face = "bold"))
```


## Visualization of policy network (comparison before and after 2017)


```{r}
pdf("mcf_Figure.pdf", width=9, height=8)

network_1316 <- correlations_filtered_1316 %>% 
  filter(correlation > 0) %>% 
  graph_from_data_frame() %>% 
  ggraph(layout = "with_kk") +
  geom_edge_link(aes(alpha = correlation)) +
...
network_1720 <- correlations_filtered_1720 %>% 
  filter(correlation > 0) %>% 
  graph_from_data_frame() %>% 
  ggraph(layout = "with_kk") +
  geom_edge_link(aes(alpha = correlation)) +
...
mcf_docs_raw %>% 
  ...
  geom_col() + 
  scale_x_continuous(breaks = seq(2013, 2020, 1)) +
  labs(title = "Figure 6. MCF policy documents by domain over time (2013-2020)", 
       x = " ",
       y = "Number of documents", 
       fill = " ", 
       caption = "Data Source: MCF Documents Dataset Ver.1") +
